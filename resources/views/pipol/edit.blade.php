    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card card-primary">
                <form action="{{ route('pipols.update', $pipol) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input value="{{ old('pipol_code', $pipol->pipol_code) }}" type="text" class="form-control @error('pipol_code') is-invalid @enderror" name="pipol_code" id="pipol_code" placeholder="PIPOL Code">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input value="{{ old('project_title', $pipol->project_title) }}" type="text" class="form-control @error('project_title') is-invalid @enderror" name="project_title" id="project_title" placeholder="Project Title">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control @error('category') is-invalid @enderror" name="category" id="category">
                                    ...
                                    @foreach($categories as $option)
                                        <option value="{{ $option }}" @if(old('category', $pipol->category) == $option) selected @endif>{{ $option }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <div class="input-group mb-3">
                                    ...
                                    <input value="{{ old('pipol_url', $pipol->pipol_url) }}" type="text" class="form-control @error('pipol_url') is-invalid @enderror" name="pipol_url" id="pipol_url" aria-describedby="pipol_url" placeholder="Project ID">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control @error('submission_status') is-invalid @enderror" name="submission_status" id="submission_status">
                                    ...
                                    @foreach($submission_statuses as $option)
                                        <option value="{{ $option }}" @if(old('submission_status', $pipol->submission_status) == $option) selected @endif>{{ $option }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-9">
                                <select class="form-control select2 @error('ipms_id') is-invalid @enderror" name="ipms_id" id="ipms_id">
                                    ...
                                    @foreach($projects as $option)
                                        <option value="{{ $option->id }}" @if(old('ipms_id', $pipol->ipms_id) == $option->id) selected @endif>{{ $option->title }}</option>
                                    @endforeach
                                </select>
