...
@section('page-header')
    ...
    @else
        <div class="flash flash-error flash-full border-top-0 text-center text-bold py-2">
            ...
            The system has closed on {{ \Carbon\Carbon::create(config('ipms.deadline'))->subSecond()->toDayDateTimeString() }}.
            It is now in read-only mode for encoders. You can no longer create/update PAPs.
        </div>
    @endif
...
@section('content')
    <div class="flash flash-error mb-5">
        <h5 class="h5 lh-condensed mb-2">Clarification on the endorsement letter generated by the system</h5>
        <p>The system-generated endorsement letter will include all PAPs of the office that were encoded
            into the PIPS, including those that were previously submitted in preceding updating of the PIP and TRIP.
            This is to ensure that all PAPs encoded are properly accounted esp. for validation
            purposes. The PAPs are categorized as follows:
        </p>
        <ol class="ml-4">
            <li>Endorsed PAPs: PAPs that were endorsed by the encoder for inclusion to the TRIP.</li>
            <li>Dropped PAPs: PAPs that were dropped by the encoder based on their stated reason.
                The indicated reason will appear in the endorsement letter. If you wish to change the reason,
                just "undo drop" the PAP.
            </li>
            <li>Neither endorsed nor dropped PAPs: PAPs that were neither dropped nor endorsed (i.e. draft). This
                is to account for non-action of the encoders on that PAP. It is understood that the office/OU did
                not submit these PAPs for inclusion in the TRIP.
            </li>
        </ol>
