...
class RefFundingSource extends Model
{
    ...
    {
        return $this->hasMany(ProjectFsInvestment::class,'fs_id','id');
    }

    public function fs_infrastructures(): HasMany
    {
        return $this->hasMany(ProjectFsInfrastructure::class,'fs_id','id');
    }
    ...
    {
        return $this->hasOne(ProjectFsInvestment::class,'fs_id')
            ->selectRaw('fs_id')
    ...
    {
        return $this->hasOne(ProjectFsInfrastructure::class,'fs_id')
            ->selectRaw('fs_id')
            ->selectRaw('count(id) as count')
            ->selectRaw('sum(y2016) as "y2016"')
            ->selectRaw('sum(y2017) as "y2017"')
